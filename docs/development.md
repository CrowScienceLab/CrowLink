...
CrowLink는 관리자 권한을 요청하거나 규칙을 자동 생성하지 않습니다. Windows 보안 경고가 표시되면 신뢰하는 사설 네트워크에 한해 허용합니다. 수동 규칙이 필요하면 TCP 45100과 UDP 45101의 인바운드를 사설 프로필과 CrowLink 실행 파일로 제한합니다. 포트를 설정에서 변경하면 규칙도 맞춰야 합니다.
...
- 1.0은 protocol version 5이므로 파일·Explorer 패키지·클립보드·키보드·마우스 시험 시 두 PC 모두 같은 버전을 사용해야 합니다.
