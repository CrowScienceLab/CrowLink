...
모든 메시지는 UTF-8 JSON입니다. 인증 전 최대 16KiB, 승인 후 최대 3MiB 프레임을 허용합니다. 공유 텍스트는 100,000자, PNG는 2MiB 이하입니다. 브라우저 내 텍스트·이미지 교환은 지원하지만 휴대폰 다른 앱에 직접 입력하지 않습니다.
...
- 펜 모드 한 손가락 누름·이동: 선택한 PC 모니터의 네이티브 오버레이 필기. 휴대폰에도 궤적과 3×3 가이드를 표시하며 PC 화면 미리보기는 전송하지 않습니다.
